...
NAN_METHOD(ODBCResult::Fetch) {
  ...
  if (info.Length() == 1 && info[0]->IsFunction()) {
    ...
    if (obj->Has(fetchModeKey) && obj->Get(fetchModeKey)->IsInt32()) {
      data->fetchMode = obj->Get(fetchModeKey)->ToInt32(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked()->Value();
    }
  }
  else {
...
NAN_METHOD(ODBCResult::FetchSync) {
  ...
  if (info.Length() == 1 && info[0]->IsObject()) {
    ...
    Local<String> fetchModeKey = Nan::New<String>(OPTION_FETCH_MODE);
    if (obj->Has(fetchModeKey) && obj->Get(fetchModeKey)->IsInt32()) {
      fetchMode = obj->Get(fetchModeKey)->ToInt32(Isolate::GetCurrent()->GetCurrentContext()).ToLocalChecked()->Value();
    }
  }
