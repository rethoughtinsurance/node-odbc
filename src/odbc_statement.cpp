...
void ODBCStatement::UV_AfterExecute(uv_work_t* req, int status) {
  ...
  if(data->result == SQL_ERROR) {
    ...
    Local<Value> js_result = Nan::New(ODBCResult::constructor)->NewInstance(Isolate::GetCurrent()->GetCurrentContext(), 4, info).ToLocalChecked();
...
NAN_METHOD(ODBCStatement::ExecuteSync) {
  ...
  if(ret == SQL_ERROR) {
    ...
    Local<Object> js_result = Nan::New(ODBCResult::constructor)->NewInstance(Isolate::GetCurrent()->GetCurrentContext(), 4, result).ToLocalChecked();
